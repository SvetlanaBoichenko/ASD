...
public class BloomFilter3Test {
    ...
    public void test_Add_remove1() {
        ...
        assertTrue(bf.isValue("ab", 1) == false);
        assertTrue(bf.isValue("abc",1) == false);
        assertTrue(bf.isValue("0123456798",1) == false);
        assertTrue(bf.isValue("0123456789",1) == true);

        bf.remove("0123456789");
        assertTrue(bf.isValue("0123456789", 1) == true);
        bf.remove("0123456789");
        assertTrue(bf.isValue("0123456789", 1) == false);
    }
    @Test
    public void test_Add_remove2() {
        ...
        assertTrue(bf2.isValue("2", 3) == true);
        bf2.remove("2");
        assertTrue(bf2.counters[2] == 2);
        assertTrue(bf2.isValue("2", 2) == true);

        bf2.remove("1");
        assertTrue(bf2.counters[1] == 0);

        assertTrue(bf2.isValue("1", 1) == false);
        assertTrue(bf2.isValue("5",1) == false);

        int mask6 = bf2.getmask("6");
        assertTrue(bf2.isValue("6", 1) == true);//ложное срабатывание
